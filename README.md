...
- 窗口模式模拟器或真实平板完整设备 QA。
- Android 语音输入真机 QA、本地 SenseVoice ASR 准确率/延迟 QA、小白狐 opening greeting、复杂动画低配 QA 和完整设备 QA。
...
- 本地 SenseVoice ASR 已作为正式 provider 接入 `/api/v1/asr/transcribe`，默认仍为 mock；启用 `local_sensevoice` 后优先本地识别，异常时按配置 fallback 到原 MiMo ASR 路径。MiMo ASR provider 保留但默认 disabled/policy-blocked；Android 已接入录音上传和儿童默认自动发送，调试模式仍可显示待确认文本。真实儿童音频外发必须由父亲授权和 ASR data policy flags 控制。
