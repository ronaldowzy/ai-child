...
- 语音输入 ASR v1 目标已修订为后端接 MiMo audio input / ASR；Android 不直接调用 MiMo，只负责录音上传和待确认文本 UI。
...
- Streaming v1 已完成首版后端和 Android client：新增 `/api/v1/conversation/stream` NDJSON pseudo streaming，旧 `/api/v1/conversation/message` 保留为 fallback。
- MiMo ASR spec 已完成脱敏 intake 和设计归档；`/api/v1/asr/transcribe` 已挂载，MiMo ASR provider 已实现但默认 disabled/policy-blocked；真实儿童音频外发必须由父亲授权和 ASR data policy flags 控制。
