...
- 窗口模式模拟器或真实平板完整设备 QA。
- Android 真实语音输入、小白狐 opening greeting、复杂动画低配 QA 和完整设备 QA。
...
- 语音输入 ASR v1 目标已修订为后端接 MiMo audio input / ASR；Android 不直接调用 MiMo，只负责点击录音、上传后端和儿童端语音状态。
...
- MiMo ASR spec 已完成脱敏 intake 和设计归档；`/api/v1/asr/transcribe` 已挂载，MiMo ASR provider 已实现但默认 disabled/policy-blocked；Android 已接入录音上传和儿童默认自动发送，调试模式仍可显示待确认文本。真实儿童音频外发必须由父亲授权和 ASR data policy flags 控制。
