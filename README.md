...
- Streaming v1 后端骨架已完成：新增 `/api/v1/conversation/stream` NDJSON pseudo streaming，旧 `/api/v1/conversation/message` 保留；Android stream client 尚未接入。
- MiMo ASR spec 已完成脱敏 intake 和设计归档；云 ASR 默认 disabled，Android 本地 SpeechRecognizer confirm-before-send 仍是 v1 默认路线。
