...
- MiMo ASR spec 已完成脱敏 intake 和设计归档；`/api/v1/asr/transcribe` 已挂载，MiMo ASR provider 已实现但默认 disabled/policy-blocked；Android 已接入录音上传和待确认文本 UI。真实儿童音频外发必须由父亲授权和 ASR data policy flags 控制。
