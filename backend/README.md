...
- Opening greeting is available at `POST /api/v1/conversation/opening`.
  ...
  fallback on provider failure. Android should treat opening/TTS as non-blocking
  and keep the first screen Ready.
...
- Open conversation now exposes `conversation_control` from the same child_chat
  ...
  safety, privacy, bedtime, explicit boundaries, fallback, and metrics.
- Topic suggestions are generated by backend quick actions from child profile
  ...
  conversation_control. Android should render backend actions and not provide
  hard-coded independent topic chips.
...
## Task 09 Auth And Personalization Thin Slice

Task 09 is a family-beta foundation, not production auth compliance:
...
- Existing conversation, opening, stream, parent policy, and parent report APIs
  accept the bearer token and prefer the authenticated child_id; explicit
  child_id remains available for local dev/test compatibility.
