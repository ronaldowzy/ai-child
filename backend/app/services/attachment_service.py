...
class AttachmentService:
    ...
    def _vision_prompt(self, request: AttachmentCreateRequest) -> str:
        ...
        return (
            ...
            "只能返回以下字段：child_summary, context_summary, recognized_type, confidence, fallback_action。"
            "child_summary 是一句孩子可理解的短摘要，不超过80个中文字符；"
            "context_summary 是供后续对话使用的受控图片上下文，不超过700个中文字符。"
            "recognized_type 只能是 image_observation、homework_problem、privacy_sensitive 或 unsafe_unknown。"
            "confidence 是 0 到 1 的数字。"
            "fallback_action 可为空；看不清题干时用 confirm_or_retake；有多道题但无法确定孩子要哪一道时用 ask_problem_number。"
            "如果图片像作业题，context_summary 必须优先保留：题号、每道题可见题干、孩子可能指定题目的线索。"
            "如果能看出有多道题，请写成：可见题目：第X题：……；第Y题：……。"
            "如果某道题看不完整，请明确写：第X题看不完整。"
            "不要把其他题的内容混到孩子指定的题号里。"
            "不要输出作业最终答案。若图片像作业题，只提取题目内容、题号和要求。"
            "如果图片用途是学习作业，哪怕题干不完整，也优先识别为 homework_problem。"
            "如果孩子补充说明里有第几题、开头几个字、哪一道等线索，请在 context_summary 里保留这个线索，但不要据此编造看不见的题干。"
            "如果图片只是普通物品、画、玩具或生活场景，按普通图片描述，不要默认当作作业。"
