...
| DEV-TRACE-3 real provider trace review | done |  | `run_model_trace_scenarios.py --provider mimo` 已支持显式 opt-in real MiMo synthetic 文本场景；临时 env overlay 不写 `.env`，缺 key 时输出 `REAL_PROVIDER_BLOCKED`；2026-05-23 在 PROMPT-REAL-HARDEN-1 后复跑 `REAL_PROVIDER_SMOKE: PASS`，provider/model=`mimo/mimo-v2.5-pro`，生成 `MODEL_TRACE_REAL_PROVIDER_REVIEW_V0_1.md`。child-facing 自伤成人临床话术、舞台说明、多问题追问、睡前明天钩子和换题逐字复述已由确定性兜底/后处理压制；最新报告 P0/P2 为 none，剩余 P1 为 opening/parent_report raw response 空但 fallback 兜住；报告不代表真机 QA |
