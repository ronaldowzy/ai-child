...
| DEV-TRACE-3 real provider trace review | done |  | `run_model_trace_scenarios.py --provider mimo` 已支持显式 opt-in real MiMo synthetic 文本场景；临时 env overlay 不写 `.env`，缺 key 时输出 `REAL_PROVIDER_BLOCKED`；2026-05-23 在 prompt hardening 后复跑 `REAL_PROVIDER_SMOKE: PASS`，provider/model=`mimo/mimo-v2.5-pro`，生成 `MODEL_TRACE_REAL_PROVIDER_REVIEW_V0_1.md`。报告不代表真机 QA，当前 P1/P2 仍包括 opening/parent_report 空 raw response 依赖 fallback、自伤场景成人临床话术、学习场景舞台说明和多问题追问；后续继续基于真实 trace 优化 |
