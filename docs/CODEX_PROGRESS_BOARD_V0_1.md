...
| R7 | 完整设备 QA | 家庭内测前完整平板手动验收 | in_progress | E2E/R1-R6 | QA1 已记录当前 v0.1+ 基础闭环事实；新增小白狐命名、语音输入确认、TTS 自动朗读、停止/静音、VoiceProfile、后端 ASR/TTS 评估和 3D/fallback 待验收项 |
...
| EXP-E1 | Android 统一交互状态 | child-facing phase reducer 统一 ASR、stream、TTS、图片、错误态和小白狐反馈 | done | V1/F1/S-Stream | 新增 `ChildTurnUiPhase` / `ChildInteractionPresentation` thin slice；InputBar 主按钮、状态短语、小白狐 agent、TTS “停一下”和图片按钮可见性从统一 presentation 派生；静音常驻入口和真机布局仍待后续 QA/收敛 |
...
| EXP-E1-01 Android unified interaction state | done |  | 新增 `ChildTurnUiPhase` / `ChildInteractionPresentation` reducer，集中派生 Ready、Listening、Recognizing、Sending、Thinking、SpeakingPending、Speaking、ImageProcessing、NeedsRetry、PermissionNeeded、Resting、ServiceError 的儿童可见状态；InputBar 主按钮文案、按钮可用性、图片按钮可见性、TTS “停一下”、小白狐状态短语和 agent 状态已接入；未改后端协议、prompt 年龄分层、图片具体反馈、父亲日报或 Healthy Engagement 指标；Redmi K60 / Honor Pad 5 真机 QA 仍待验 |
