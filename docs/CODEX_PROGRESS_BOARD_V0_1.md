...
| HE-05 Healthy Engagement observability | done |  | Task 04 thin slice：后端在 runtime/conversation/stream 链路生成 `healthy_engagement_turn` / `healthy_engagement_stream` 非原文结构化日志，覆盖 boundary_signal、boundary_respected、question_count、consecutive_recent_questions、age_band、scene、reply_char_count 和 first_text/first_audio/turn_total timing；测试确认边界/睡前/连续追问、payload 不含儿童原文或回复全文、telemetry 失败不阻断响应；不把 Healthy Engagement 写成最终完成 |
