...
| DB1-E ParentReport 持久化 | done |  | thin slice done：ParentReportService 优先从 PostgreSQL `parent_reports` 读取已生成日报，缺失时基于结构化 memory 生成并保存；数据库不可用时回退即时生成/进程内缓存；不保存 evidence、quote_summary、raw transcript、prompt、debug 或 provider raw response |
...
| QA-Smoke-2 本地 DB/ASR/Vision smoke 收口 | done |  | 新增 `setup_local_postgres.sh`、`check_asr_real_status.sh` 和 `smoke_vision_model_opt_in.sh`；本机无 Docker CLI 时已自动安装/启动 Homebrew postgresql@16，并完成 migration + DB persistence smoke PASS；ASR 状态脚本确认当前 `.env` 仍是 mock_only；MiMo vision/OCR code path 已实现但 real smoke BLOCKED，缺 `CHILD_AI_MIMO_ALLOW_IMAGE=true` 和非儿童测试图片路径 |
...
阻塞问题：上一轮本机 PostgreSQL 未启动，`smoke_db_persistence.sh` 按设计输出 SKIP，不计为通过；MiMo ASR real smoke 未执行，因为本轮未设置全部 opt-in policy env、MiMo key 和非儿童测试音频路径；真机 base URL APK 未构建，Redmi K60 / Honor Pad 5 未安装验证。
...
完成任务：新增本地 PostgreSQL setup 脚本，优先 Docker Compose，Docker 不可用时在 macOS 尝试 Homebrew postgresql@16，并在成功后自动运行 migration 与 DB persistence smoke；新增 ASR 状态检查脚本，明确区分 mock_only、policy_blocked、mimo_ready、mimo_smoke_pass、mimo_smoke_fail；新增 OpenAI-compatible multimodal image payload 支持、AttachmentService image_data_uri 最小 vision path 和 vision opt-in smoke 脚本。默认仍为 MockOCR，不接 CameraX，不保存 raw image/base64。
阻塞问题：ASR real smoke 未执行，因为当前 `.env` 中 `CHILD_AI_ASR_PROVIDER` 不是 `mimo`；vision real smoke 未执行，因为缺 `CHILD_AI_MIMO_ALLOW_IMAGE=true` 和 `CHILD_AI_VISION_SMOKE_IMAGE` 非儿童测试图片路径；Redmi K60 / Honor Pad 5 仍未真机 QA。
...
需要补充到 AGENTS.md 的规则：暂无。
明日第一任务：按本轮 smoke 结果修正本地环境；如 real MiMo ASR/vision blocked，补齐 env 和非儿童测试素材后复跑；随后进入 Redmi K60 / Honor Pad 5 真机 QA。
```
