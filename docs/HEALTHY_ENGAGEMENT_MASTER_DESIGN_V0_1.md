...
当前项目已经从“功能能跑通”进入“长期体验设计”阶段。voice-first、opening greeting、MiMo TTS、MiMo ASR smoke、MiMo vision smoke、stream pseudo streaming、PostgreSQL 本地持久化、Prompt Guidance v0.1、家长日报等能力已经形成第一版闭环。下一阶段的核心不应是简单堆功能，而是设计一种健康、可持续、非成瘾的儿童主动回访机制。
...
10. 家长日报：结构化 memory + conversation persistence 生成日报。
```
...
5. 家长日报已有分析，但尚未形成“父亲今晚可说的一句话”机制。
...
4. 模型回复避免成人式审问和诊断。
5. Backend age-banded replies thin slice 已把 age_5_6 / age_7_8 / age_9_10 / unknown 的长度预算作为内部 prompt 和 runtime 约束；默认 age_7_8，不再把 8 岁写死成唯一基线。
```
...
5. 连续追问 throttle thin slice 已在 open conversation 生效：最近小白狐连续提问、孩子表达边界或纠正误听时，本轮先回应/陈述/收束，不新增问题钩子。
```
...
Tests: E2-A 覆盖 interest callback、boundary respect、bedtime closure/defer、no-school policy、家长目标低压力转译、分龄和 memory failure fallback。
...
Goal: 防止无限追问，支持换轨和收束。
Backend: 已完成 thin slice：TurnGuidanceBuilder 检测连续问句、换题/不聊/睡觉边界和孩子纠正，ChildAgentRuntime 对普通开放对话移除新追问钩子；完整 ConversationArcStateService 仍后续。
...
Android: Task 03 thin slice 已在家长日报 UI 顶部增加“今晚可以怎么接一句”，失败态改为家庭化文案；儿童聊天页家长入口默认降噪为小“大人”按钮，长按 + PIN 后仍可进家长日报/设置。
Prompt: 家长端不是监控，而是支持。
Tests: 已覆盖 bridge 字段解析、空素材不追问、失败态不暴露工程词、固定负面标签替换和不返回逐字记录；仍需真实模型自然度观察。
Acceptance: 家长日报首屏能给现实接话建议，且不把报告做成监控面板。
...
4. 家长日报增加 `avoid_followup`，提醒父亲不要追问逐字聊天、旧话题、游戏时长/输赢或作业最终答案。
```
...
3. 家长日报的 avoid_followup 是否帮助现实沟通，而不是让父亲过度监控。
```
...
3. 家长日报的 CS/game synthetic case 会归纳地图、队友/朋友配合和输赢感受，但仍只显示 summary、bridge 和 avoid_followup，不展示逐字聊天记录、provider/debug 词或行为评分。
4. TTS latency observability 只记录 request_id、hash、阶段耗时和播放状态，帮助判断慢在 model、TTS 生成、stream audio_ready 还是 Android playback start；不得记录儿童原文、完整回复、原始音频、完整 audio URL 或家长寄语原文。
```

## 19. Task 09 Model-driven Control And Account Context

Task 09 后的 Healthy Engagement 相关边界：

```text
1. `conversation_control` 由 child_chat 同一次模型调用给出，表达孩子参与度、继续/soft_shift/stop 判断、换题意图和建议动作；程序规则仍覆盖安全、隐私、睡前、不聊/换话题等明确边界。
2. 对 CS/game/sports 这类孩子短答场景，模型可语义判断低能量并 soft_shift；对孩子主动补细节的高参与场景，不因同题轮数机械换题。
