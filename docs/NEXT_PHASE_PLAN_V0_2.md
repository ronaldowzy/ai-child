...
18. 父亲已确认 ASR v1 第一选择改为本地 SenseVoice；MiMo audio input / ASR 作为 fallback，真实儿童音频外发必须由父亲授权和 ASR data policy flags 控制，开发阶段先用 fake/smoke audio 或非儿童测试音频。
19. 下一阶段必须补齐 request_id、结构化日志、provider timing、health 扩展、环境检查和 QA 记录等运行基础组件。
20. 最新产品方向修订为 freedom-first：默认自由对话，时间、家长寄语、记忆、最近聊天和图片能力作为上下文或工具；高风险安全、隐私边界、明确学习求助、明确睡前收尾和父母强规则作为护栏。
21. 拍照能力从“拍题目”升级为“拍给小白狐看”的通用图片分享；玩具、画、书、植物、手工和作业都应先理解孩子意图，再分流。
22. 家长寄语需要支持自由文本，作为 Prompt 背景上下文注入；不能机械复述给孩子，不能覆盖儿童安全底线。
...
28. Android 家长设置页已支持结构化配置孩子小名和显示名；opening greeting 使用小名优先、显示名 fallback，但真机 QA 仍待完成。
...
```text
1. Redmi K60 / Honor Pad 5 全量真机 QA 仍为 NOT_RUN；负责人：家长/设备测试；下一步：按 QA_FAMILY_BETA_CHECKLIST_V0_1.md 逐行记录。
2. 真实儿童 ASR 准确率 NOT_VALIDATED；负责人：家长/设备测试；下一步：仅在授权和数据策略满足时用非敏感测试语音记录准确率与延迟。
3. real-provider naturalness NEEDS_REVIEW；负责人：后端/prompt；下一步：人工复核 age-band、question throttle、creative-share 和 parent_report trace，不用 mock pass 代替。
4. ParentReport model-first 仍需真实家庭语境 review；负责人：后端/prompt + 父亲；下一步：复查超时/fallback 和“今晚可以怎么接一句”自然度。
...
目标：让孩子以语音作为默认输入，Android 上传短音频到后端 ASR；后端优先本地 SenseVoice 识别，本地异常后 fallback 到原有 MiMo ASR；儿童默认自动发送 transcript 进入 conversation。confirm-before-send 只保留为 DevSettings / 父亲调试模式；v1 不做 hands-free conversational mode。
...
1. 不做常开麦克风。
2. 不做唤醒词。
3. 不做 Android 直连 MiMo。
4. 不做真实儿童音频 smoke；开发阶段只用 fake/smoke audio。
...
7. ParentReportService 已优先从 PostgreSQL `parent_reports` 读取已生成日报；缺失时继续基于结构化 memory 生成并 best-effort 保存；数据库不可用时不阻断日报返回。
```
...
4. B5：ParentReportService thin slice 已完成，parent_reports 只保存结构化 summary、observations、safety_alerts 和 suggested_parent_actions。
```
...
5. 横屏改造不得破坏 MiMo audioUrl 播放、animation_v1 和现有家长入口保护。
```
...
7. Android 家长设置增加家长寄语，图片入口改为“拍给小白狐看”。
8. 第二轮收窄学习意图，不再用单独“题 / 不会”触发 homework。
9. 普通图片后续快捷动作携带 attachment_id 和图片摘要，进入 LLM 上下文。
10. ParentPolicyService 优先使用 PostgreSQL 持久化家长寄语，dev/test 数据库不可用时回退内存。
```
...
家长寄语进入 Prompt，但不出现在儿童 UI debug。
```
...
3. 如果未来持久化完整对话，必须单独做儿童数据、家长治理、删除和最小化审查。
```
