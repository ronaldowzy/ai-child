...
```text
1. Redmi K60 / Honor Pad 5 全量真机 QA 仍为 NOT_RUN；负责人：父亲/设备测试；下一步：按 QA_FAMILY_BETA_CHECKLIST_V0_1.md 逐行记录。
2. 真实儿童 ASR 准确率 NOT_VALIDATED；负责人：父亲/设备测试；下一步：仅在授权和数据策略满足时用非敏感测试语音记录准确率与延迟。
