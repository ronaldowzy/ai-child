...
Task 10 closes the first QA gap around that foundation:

```text
1. Authenticated child_id ownership is covered for conversation, stream, opening, parent policy, parent report, and attachment upload paths.
2. SharedPreferences token remains a family-beta thin slice and must not be described as production auth hardening.
3. Opening v2 has non-sensitive latency timing and TTS soft timeout so text opening is not held hostage by audio generation.
4. Model conversation_control and backend topic choices have synthetic trace coverage for CS/game soft_shift, high-detail continue, stop guardrail, interest choices, and topic boundaries.
5. Redmi K60 / Honor Pad 5 real-device QA is still NOT_RUN.
```
...
2. Task 10: account/opening/control closeout and QA package.
3. Device QA round after Task 10 on Redmi K60 and Honor Pad 5.
4. Fix only P0/P1/P2 issues from actual device evidence.
5. Improve TTS latency only after logs identify bottleneck.
6. Keep parent report summary understandable but not transcript-based.
```
