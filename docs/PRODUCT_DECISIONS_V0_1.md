...
2. 语音输入 v1：后端接 MiMo ASR，Android 负责录音上传和儿童端语音状态；儿童默认自动发送非空 transcript，确认面板仅用于 DevSettings / 父亲调试模式；仍不做常开麦克风或自动连续监听。
...
13. 下一阶段体验重点转向流式交互、横屏双栏、小白狐状态覆盖验证、MiMo ASR 后端接入、儿童端 voice-first 自动发送、opening greeting 和运行基础组件补齐。
