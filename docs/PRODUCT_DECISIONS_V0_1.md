...
| PD-021 | confirmed | Redmi K60 / Android 14 真机反馈显示 TTS 链路不可观测且无声，下一步优先修 TTS 诊断、UI 状态和 fallback，再决定是否继续依赖系统 TTS。 | Android TTS、VoiceProfile、QA、下一阶段计划 |
...
| PD-024 | confirmed | 下一版对话方向是 Open Conversation Mode / Freer Context Mode：普通聊天更自由，模型接收最近多轮上下文，SceneOrchestrator 保留安全、隐私、学习和睡前约束。 | 后端设计、ConversationHistoryService、Prompt/Runtime、QA |
...
Decision: Redmi K60 / Android 14 真机反馈显示 TTS 链路不可观测且无声，下一步优先修 TTS 诊断、UI 状态和 fallback，再决定是否继续依赖系统 TTS。
...
Decision: 下一版对话方向是 Open Conversation Mode / Freer Context Mode：普通聊天更自由，模型接收最近多轮上下文，SceneOrchestrator 保留安全、隐私、学习和睡前约束。
...
Implementation notes: 本轮只做设计计划，不直接大改后端；后续引入 ConversationHistoryService 并控制 token、隐私和长期记忆边界。
Docs updated: `docs/NEXT_PHASE_PLAN_V0_2.md`、`docs/CODEX_PROGRESS_BOARD_V0_1.md`。
...
10. Redmi K60 / Android 14 真机反馈表明 TTS 链路需要先可观测和可降级，再评估是否继续依赖系统 TTS。
11. 下一版普通聊天方向是 Open Conversation Mode：更自由地使用最近多轮上下文，但不放松安全、隐私、学习和睡前边界。
12. 所有新增体验继续遵守儿童安全底线和数据最小化原则。
```
...
| PO-004 | proposed | 小白狐专属音色是否需要采购、训练或接入第三方音色。 | v2 再评估；v1 先修通 Android 系统 TTS 可观测链路，但系统 TTS 不作为最终音色承诺。 |
