...
Status: confirmed
Source: father / product direction
Decision: 语音输入开始进入方案准备阶段，优先调研 MiMo 是否提供 ASR / audio input 能力；在接口和儿童语音数据边界确认前，不直接实现或默认上传原始音频。
Rationale: 家庭真实使用大多数时间会是语音交互，但 ASR 涉及麦克风权限、儿童语音、数据留存、延迟和中国大陆 Android 系统能力差异，必须先确认能力和边界。
Affected modules: ASR research docs、Android voice abstraction、future SpeechInputController、TTS/stream UX、QA。
Implementation notes: 新增 `docs/ASR_INPUT_RESEARCH_V0_1.md`；继续保留 confirm-before-send 原则；若未来使用云端 ASR，必须新增数据策略 guard，不让 Android 保存模型 API key。
...
13. 下一阶段体验重点转向流式交互、横屏双栏、小白狐状态覆盖验证、语音输入调研和运行基础组件补齐。
