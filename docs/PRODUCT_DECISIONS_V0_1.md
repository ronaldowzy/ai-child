...
用途：记录家长 / 产品负责人已经确认的产品决策、边界原则和阶段调整。子会话实现前必须先检查本文件，避免新想法只留在聊天记录里。

状态值：

```text
proposed：已提出但尚未确认，不能作为实现依据。
confirmed：家长 / 产品负责人已确认，代码和文档不得冲突。
revised：已被家长 / 产品负责人修订，后续以修订后的内容为准。
...
```text
1. 家长 / 产品负责人确认的新想法、方案调整、边界原则，必须写入本文件。
...
6. 每次家长确认的新产品想法，必须先写入本文件，再进入子会话实现。
```
...
| PD-008 | confirmed | 每次家长确认的新产品想法，必须先写入 `docs/PRODUCT_DECISIONS_V0_1.md`，再进入子会话实现。 | 协作流程、AGENTS、README |
...
| PD-011 | revised | TTS 默认自动朗读小白狐回复；必须有停止/静音、DevSettings 或家长设置开关。系统 `VoiceProfile` 保留为 fallback，正式品牌音色由 PD-027 的 MiMo VoiceClone 承担。 | Android TTS、家长治理、DevSettings、后端 TTS、QA |
...
| PD-028 | confirmed | 产品底座修订为 freedom-first：默认自由对话，时间、家长寄语、记忆和图片作为上下文或能力，安全/隐私/学习/睡前强边界作为护栏。 | 后端路由、Prompt、Android 入口、QA |
| PD-029 | confirmed | 拍照从“拍作业”升级为“拍给小白狐看”的通用图片分享能力；作业题只是图片能力的一个分支。 | Attachment API、Android mock 图片、Prompt、QA |
| PD-030 | confirmed | 家长寄语需要支持自由文本，作为 Prompt 重要上下文；不能机械复述给孩子，不能覆盖儿童安全底线。 | ParentPolicy、PromptManager、Android 家长设置、QA |
...
| PD-037 | confirmed | App 打开儿童聊天页后，小白狐应主动请求 opening greeting，基于时间、家长寄语和孩子称呼生成一句短开场白；称呼优先 child_nickname，其次 child_display_name，都没有则不强行称呼。 | backend opening API、Android ChatViewModel、ParentPolicy、TTS、QA |
...
| PD-040 | revised | 家长日报在父亲点开时应结合当天已落库会话消息、路由摘要和结构化 memory，由 `ModelTaskType.PARENT_REPORT` 大模型总结生成；当天有新会话素材时刷新已有日报，但仍不展示逐字聊天记录。 | ParentReportService、ModelRegistry、ConversationPersistenceRepository、parent_reports、Android ParentReportScreen |
...
| PD-049 | revised | 家庭 MVP 前 opening greeting 默认走 deterministic policy/template 主路径；家长日报已由 PD-052 修订为 model-first。 | OpeningService、OpeningPolicyBuilder、QA |
| PD-052 | confirmed | 家长日报 v2 必须 model-first：程序只构造当天受控 evidence packet 并调用 `ModelTaskType.PARENT_REPORT`，模型结构化 JSON 才是正式日报；失败时返回明确可重试状态，不用规则日报冒充成功。 | ParentReportService、ModelRegistry、ParentReportScreen、parent_reports |
...
Decision: 每次家长确认的新产品想法，必须先写入 `docs/PRODUCT_DECISIONS_V0_1.md`，再进入子会话实现。
...
Decision: TTS 默认自动朗读小白狐回复；必须有停止/静音、DevSettings 或家长设置开关。系统 `VoiceProfile` 保留为 fallback，正式品牌音色由 PD-027 的 MiMo VoiceClone 承担。
...
Implementation notes: 当前不接第三方 TTS；如使用云 TTS，必须新增 TTS data policy guard，并由家长确认供应商、留存、训练、删除、费用和稳定性。
...
Implementation notes: 已先实现小步后端版本：普通话题进入 `conversation.open`，PromptManager 使用开放对话 prompt，ChildAgentRuntime 传入进程内短期 history；不把完整聊天写入长期 memory 或家长日报。
...
Tests or QA needed: 同 session 连续普通聊天能利用上下文；家长日报不展示逐字聊天记录。
...
Decision: 本地持久化数据库选择 PostgreSQL。v0.1-dev 先作为家庭自用本地库，允许交互文本进入本地 PostgreSQL，用于上下文、复盘、家长日报和后续体验优化；原始音频、原始照片、API key 和生产 secret 仍不得入库。
Rationale: 当前内存态服务重启会丢失家长设置、会话、记忆和日报素材；家庭内测前需要本地可追溯数据基础，同时仍保持本地优先和数据最小化。
...
Decision: App 打开儿童聊天页后，小白狐应主动请求 opening greeting，基于时间、家长寄语和孩子称呼生成一句短开场白；称呼优先 `child_nickname`，其次 `child_display_name`，都没有则不强行称呼。
Rationale: 孩子打开 App 时需要小白狐自然进入状态，至少在已配置称呼时喊一下孩子，而不是空白等待或固定模板。
Affected modules: backend opening API、ParentPolicy schema、Prompt/opening service、Android ChatViewModel、Android parent settings、remote audio playback、QA。
Implementation notes: 后端新增 `POST /api/v1/conversation/opening`；同一 session 做短期去重；TTS 失败不影响文本 opening。Android 每个 chat session 只请求一次，如果孩子先开始说话，opening 不应打断或覆盖孩子输入。家长设置页已支持配置 `child_nickname` 和 `child_display_name`，文案不诱导填写真实全名。
Docs updated: `backend/README.md`、`android/README.md`、`docs/MANUAL_QA_V0_1.md`、`docs/CODEX_PROGRESS_BOARD_V0_1.md`。
Tests or QA needed: 小名优先、display name fallback、无名称不强称呼、晚上低刺激、家长寄语不查岗、opening 音频播放、用户先说话时不插入；Redmi K60 / Honor Pad 5 真机复验家长设置保存后的 opening 称呼。
...
Decision: 家长日报在父亲点开时应结合当天已落库会话消息、路由摘要和结构化 memory，由 `ModelTaskType.PARENT_REPORT` 大模型总结生成；当天有新会话素材时刷新已有日报，但仍不展示逐字聊天记录。
...
Implementation notes: 程序只读取当天会话、routing/scene/risk 受控信号和 parent-visible memories，构造最小 evidence packet；模型输出必须是结构化 JSON。程序不按规则拼正式日报正文、学习观察、表达观察、情绪观察或父亲建议；provider fail / policy block / 空输出 / JSON 不可解析时返回 `generation_status=model_failed|model_blocked`，家长端显示稍后重试，不用 deterministic fallback 冒充成功。
...
Decision: 图片上传后的儿童端确认语必须短，不向孩子回显完整 vision/OCR 描述；家长日报由大模型基于当天会话受控素材生成，规则摘要仅可作为开发 fallback 或内部 evidence hints，不能作为正式成功日报展示。
Rationale: 真机 QA 发现图片识别内容直接展示给孩子会过长、包含不适合儿童端看到的分析细节，并且可能导致单次 TTS 失败；家长日报如果只按规则分类，会空泛，不能表达孩子当天真实状态。
...
Docs updated: `docs/PRODUCT_DECISIONS_V0_1.md`。
Tests or QA needed: 图片上传确认语短且可朗读；后续图片聊天不再说“看不到”；家长日报能基于当天新对话由模型生成具体状态分析，模型失败时家长端显示可重试状态；Redmi K60 / Honor Pad 5 仍需真机复测。
...
Decision: child_chat prompt 必须显式理解儿童语音里的操作旁白、夸张表达、ASR 误听、话题换轨和睡前收尾；“跑完要死了/累死了”这类运动语境表达按 watch-lite 疲惫/身体感受处理，不直接等同自伤；家长日报 topic 抽取不得把运动比赛误报为学习求助。
...
Rationale: 家庭内测前已经跑通 voice-first、TTS、ASR、vision、stream、DB、prompt guidance 和家长日报的基础闭环；下一阶段必须防止为了留存而引入成瘾式机制或排他关系。
...
Decision: Opening Greeting v2 使用后端 policy engine，而不是只靠 prompt 或“有 seed 就回访”。Opening 必须是轻邀请、可退出、尊重边界、睡前收束，并把家长目标转译为低压力提示。
...
Decision: 家庭 MVP 前 `opening greeting` 默认走 deterministic policy/template 主路径；Task 09 后 opening 默认路径已由 PD-057 修订为 model-generated v2 with deterministic fallback；家长日报在 PD-052 中已修订为 model-first，本决策不再约束家长日报。
Rationale: DEV-TRACE-3 与 PROMPT-REAL-HARDEN-1 的真实 MiMo synthetic trace 显示 `child_chat` 质量可通过 prompt/output safety 加固继续观察；opening 继续使用确定性 policy/template 保证儿童端稳定。家长日报的产品要求已变为大模型总结，因此从 deterministic-default 决策中移出。
Affected modules: OpeningService、OpeningPolicyBuilder、backend docs、opening tests。
Implementation notes: 历史实现曾让 `OpeningService.create_opening()` 默认使用 `OpeningPolicyBuilder` + deterministic templates，不默认调用 `ModelRegistry.generate()`；Task 09 已改为安全可用时调用 model opening v2，失败时回到 deterministic templates。家长日报已从该 deterministic-default 规则中移除。
...
Tests or QA needed: 后端测试覆盖 opening model v2、deterministic fallback、trace runner opening path、Redmi K60 / Honor Pad 5 真机 QA 仍未完成。
...
Decision: 家长日报 v2 必须 model-first。正式日报正文、学习观察、表达观察、情绪观察、安全提醒和家长建议来自 `ModelTaskType.PARENT_REPORT` 的结构化模型输出；程序只负责构造当天受控 evidence packet、校验 schema、持久化模型日报和刷新 stale report。
Rationale: 家长日报需要基于当天真实对话总结孩子状态和建议，规则拼接会空泛，且会把 deterministic fallback 误当成正式成功。模型不可用时应明确失败/可重试，而不是展示一份程序拼出来的“成功日报”。
Affected modules: ParentReportService、ModelRegistry、ParentReportRepository、ParentReportScreen、docs/tests。
Implementation notes: `ParentReportService` 读取当天 `conversation_messages`、routing/active_scene/risk 受控信号和 parent-visible memories，构造不含 provider raw、debug trace、prompt、secret、base64 或原始媒体的 evidence packet。合法模型 JSON 才保存为 `generation_status=model_generated`；policy blocked、provider fail、空输出或 JSON 不可解析返回 `model_blocked/model_failed`，Android 家长端显示“日报暂时生成失败，请稍后重试”，不展示 deterministic fallback 正文。`mimo_parent_report` 默认保留 `mimo-v2.5-pro`，但使用独立 completion budget，避免沿用 child_chat 的短回复 token 限制导致空输出。Opening 默认路径已由 PD-057 修订为 model-generated v2 with deterministic fallback。
...
Implementation notes: Android 使用 `ActivityResultContracts.TakePicture` + `FileProvider` 或系统相册选择，压缩为 JPEG bytes 后 multipart 上传 `POST /api/v1/attachments/images`；后端短期保存图片测试文件并调用 `mimo_vision`。普通纯文字 child_chat 继续使用 `mimo-v2.5-pro`；图片/识图/OCR/multimodal 使用 `mimo-v2.5`。Opening 默认路径已由 PD-057 修订为 model-generated v2 with deterministic fallback；ParentReport 按 PD-052 改为 model-first。Android 不保存 MiMo key。
...
Date: 2026-05-24
Status: confirmed
Source: father / post-device QA product refinement Task 06
Decision: 家庭内测 v0.1 的家长设置改为孩子画像优先：保留小名/显示名，强烈建议填写年龄，年级可选，称呼/代词偏好、兴趣和不想被追问的话题可选；作息时间不再作为显性主配置或硬锁场景，只保留为温和上下文。普通对话连续围绕同一话题且孩子回复变短时，小白狐应提供换题机会和静态 curated seed，不继续深挖旧话题。家长日报应展示“今日聊了什么”、内容主线、今晚现实接话和避免追问点，不展示逐字聊天记录。
Rationale: 真机体验反馈显示家长设置和日报过于机械，儿童端话题容易被追问过深，聊天页也需要更稳定的小白狐陪伴感。本轮优化目标是降低家长设置负担、减少儿童被盘问感、让日报更像现实桥接工具。
Affected modules: Android ParentSettingsScreen / ParentPolicyViewModel、PromptManager、TurnGuidanceBuilder、ChildAgentRuntime、ParentReportService/domain、Android ParentReportScreen、ChildChatScreen、docs/tests。
Implementation notes: 年龄、年级、称呼偏好、兴趣和 topic boundaries 先写入 `communication_preferences`，避免新增 DB migration；topic seed pack 使用仓库静态 JSON，不抓实时网络热点；家长日报新字段保持向后兼容，旧报告没有新字段时 Android 仍可展示；小白狐 UI 只增加 phase-derived 状态 chip 和轻背景，不引入积分、签到、排行榜或宠物机制。
Tests or QA needed: 后端单测覆盖 profile prompt、topic shift seeds、runtime 换题修复和家长日报 topic overview；Android JVM 测试覆盖家长设置 profile 保存、日报 DTO/UI 文案和 child UI chip label。Redmi K60 / Honor Pad 5 仍需验证设置页可读性、话题切换自然度、日报信息层级和横屏 UI polish。
...
Date: 2026-05-24
Status: confirmed
Source: father / Task 07 closeout and latency hardening
Decision: Task 06 closeout 后，家庭内测 v0.1 继续收紧四点：隐藏 schedule/time-period 设置只作为后端兼容字段保留，家长设置保存不得因为隐藏默认时间而校验失败或重写默认作息；topic seed pack 只能使用仓库内 reviewed、age-aware、带 safety_notes 和 expires_at 的静态 curated seeds，不做实时热点抓取；TTS 延迟必须通过后端 request_id timing 和 Android logcat timing 分解 model/TTS/audio playback；家长日报继续只展示主题/内容摘要、现实接话和今晚避免追问，不提供 raw transcript 导出。
Rationale: Task 06 方向正确，但还需要关闭隐藏作息副作用、避免“趋势话题”滑向实时抓取或留存诱导、让 TTS 慢的问题有可诊断证据，并让 CS/game 日报在真实家庭语境下更自然。
Affected modules: Android ParentPolicyViewModel / ParentSettingsScreen、TopicSeedService / seed pack / TurnGuidanceBuilder、ChildChatScreen topic chips、ConversationService / ConversationStreamService / Android remote audio timing、ParentReportService / ParentReportScreen、QA docs。
Implementation notes: schedule 仍随家长策略 API 透传兼容旧数据，但 v0.1 UI 不展示且保存只保留后端已有 schedule；seed pack 每条 seed 必须包含 `id`、`label`、`age_bands`、`prompt_hint`、`safety_notes`、`expires_at`、`source=curated_v0_1`，过期或包含购买/排行/抽卡等 unsafe marker 的 seed 不返回；Android 只加轻量 idle fallback chips，不新增任务菜单、奖励或连续使用钩子；TTS timing logs 不记录儿童原文、完整回复、音频 URL query、parent_message_raw 或 provider secret。
Tests or QA needed: 自动测试覆盖 hidden schedule 不阻断保存、seed pack shape/expiry/safety、CS-like low engagement topic shift、stream/non-stream latency logs privacy、Android segment trace id propagation、CS/game parent report summary。Redmi K60 / Honor Pad 5 仍需验证设置页、换题 chips、TTS timing 抓取和日报阅读层级。
...
Date: 2026-05-25
Status: confirmed
Source: Task 09 child account and model-driven personalization
Decision: Task 09 后，家庭内测账号和个性化采用薄片实现：一个孩子一个账号，由家长代创建、登录、管理和退出；孩子不管理账号密码；Android 登录后保持登录态，除非家长手动退出或 token 失效；产品默认术语改为“家长”，代码中的 `Parent*` 类名暂不做危险大规模重命名；普通对话 continuation/shift 改为模型语义控制为主，程序规则只负责 safety、privacy、bedtime、explicit boundary、fallback 和 metrics；opening v2 可以使用模型生成并保留确定性 fallback，Android 首屏 Ready 不被 opening/TTS 阻塞；topic choices 必须由后端基于孩子兴趣、历史、curated seeds 和 conversation_control 生成，Android 不再固定写死话题 chips。
Rationale: 家庭内测已经从单一 dev child_id 进入真实账号、设置、历史、开场和日报归属阶段。继续使用父亲专属文案、Android 静态 chips 或规则主导换题，会限制后续个性化体验并增加维护分散状态。
Affected modules: backend auth API/service/repository/schema/DB migration、Android Auth screen/session store/API clients/NavHost、ParentPolicy/ParentReport copy、ChildAgentRuntime conversation_control、OpeningService/OpeningPolicy、TopicSeedService/QuickActionService、docs/QA。
Implementation notes: `child_accounts` 保存账号身份和基础画像，`ParentPolicy` 继续作为偏好/画像主存储；密码使用 PBKDF2-SHA256 hash，不保存明文；`auth_sessions` 只保存 bearer token hash、过期和撤销状态；已有 dev child_id/API 显式 child_id 路径保留兼容。Opening v2 的模型输出必须短、儿童安全、可被输出安全检查和 fallback 兜住；topic choices 使用仓库内 curated/static seeds，不抓实时热点，不做积分、签到、排行榜、抽卡、宠物饥饿值或任务压力。
Tests or QA needed: 自动测试覆盖 register/login/me/logout、明文密码/token 不入库、conversation_control soft_shift/high-engagement/guardrail/fallback、opening profile interest 和 provider fallback、topic choices interest priority/boundary filtering、Android auth DTO/session 和 no hard-coded fallback chips。Redmi K60 / Honor Pad 5 仍需真机验证登录注册、持久登录、退出、家长设置/日报使用当前账号密码复验、opening v2 不阻塞、topic choices 可读且不挤压 voice-first 控件。
...
Date: 2026-05-25
Status: confirmed
Source: parent / product owner runtime feedback
...
Date: 2026-05-25
Status: confirmed
Source: parent / runtime feedback
Decision: 家庭内测 v0.1 中，孩子基础资料以 `children` 表作为单一写入源：注册时填写的小名、显示名、年龄、年级、称呼偏好、兴趣和话题边界，以及家长设置中修改的同类资料，都必须写回同一个 child profile；`parent_policies` 继续保存家长目标、寄语、安全规则、schedule 等治理策略，不再作为孩子基础资料的主存储。家长日报保持 model-first，但父亲/家长可接受前台等待到约 90 秒，只要能生成正式模型日报；因此 parent_report 正式模型默认 timeout 为 90000ms、completion budget 为 6000，并要求 prompt 使用结构化摘要压缩，不发送逐字聊天记录。
Rationale: 运行反馈显示注册资料和家长设置资料分散会导致家长设置页看不到注册时填写的信息；同时 parent_report 15 秒窗口会连续失败，而真实 MiMo 在更长窗口和更高 completion budget 下可以产出 `model_generated` 日报。
...
```text
1. 先完成完整设备 QA，确认现有文字聊天、学习引导、系统相机真实图片上传/识图、家长治理和安全场景在平板上可用。
2. 语音输入 v1：后端本地 ASR 优先，第一选择是 sherpa-onnx + SenseVoice-Small int8；本地异常后 fallback 到原有 MiMo ASR。Android 负责录音上传和儿童端语音状态；儿童默认自动发送非空 transcript，确认面板仅用于 DevSettings / 家长调试模式；仍不做常开麦克风或自动连续监听。
3. 小白狐正式语音输出改为后端 MiMo VoiceClone 生成 `audio_url`，Android 优先播放远端音频；系统 TextToSpeech 仅保留为开发诊断/未来评估背景，不作为儿童端自动朗读 fallback。
...
12. 本地持久化方向改为 PostgreSQL：先做家庭自用本地库和最小可用表，再逐步迁移家长策略、会话消息、结构化记忆和日报；如果未来云端化或上架，必须重新做儿童数据合规评审。
...
15. Task 06/07 后，家长设置以孩子画像和沟通偏好为主；显性作息配置从 v0.1 家庭内测 UI 中降级，隐藏 schedule 只做后端兼容且不重写默认时间；普通对话同题低能量时使用静态 reviewed seed 给换题选择；家长日报以话题/内容摘要和现实接话为主，不做监控式原文回放。
16. Task 09 后，默认产品路径需要先由家长创建或登录一个孩子账号；设置、日报、opening、conversation 和 topic choices 使用该账号 child_id。账号/auth 仍是本地家庭内测薄片，不代表生产级合规完成。
17. 2026-05-25 运行反馈后，孩子基础资料收口到 `children` 表作为单一写入源；家长日报 model-first 允许约 90 秒正式模型等待窗口，但必须使用结构化摘要压缩 prompt，不发送逐字聊天记录。
```
