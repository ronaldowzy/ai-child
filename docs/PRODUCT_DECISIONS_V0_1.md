...
Decision: 图片上传后的儿童端确认语必须短，不向孩子回显完整 vision/OCR 描述；父亲日报应优先由大模型基于当天会话受控素材生成，规则摘要只作为模型不可用或解析失败时的兜底。
...
Docs updated: `docs/PRODUCT_DECISIONS_V0_1.md`。
Tests or QA needed: 图片上传确认语短且可朗读；后续图片聊天不再说“看不到”；父亲日报能基于当天新对话生成具体状态分析；Redmi K60 / Honor Pad 5 仍需真机复测。
