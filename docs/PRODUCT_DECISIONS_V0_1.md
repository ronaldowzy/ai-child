...
| PD-056 | confirmed | Task 07 closeout：父亲设置隐藏 schedule 只保留后端兼容，不再校验或重写隐藏默认时间；topic seeds 必须是 reviewed/age-aware/expiring/static，不做实时热点；TTS 延迟必须用 backend request_id 与 Android logcat timing 分解；父亲日报继续是摘要和现实桥接，不展示 raw transcript。 | ParentSettings、TopicSeedService、ChildChatScreen、Conversation/Stream/TTS timing、ParentReport、QA |
...
Decision: 家庭内测 v0.1 的父亲设置改为孩子画像优先：保留小名/显示名，强烈建议填写年龄，年级可选，称呼/代词偏好、兴趣和不想被追问的话题可选；作息时间不再作为显性主配置或硬锁场景，只保留为温和上下文。普通对话连续围绕同一话题且孩子回复变短时，小白狐应提供换题机会和静态 curated seed，不继续深挖旧话题。父亲日报应展示“今日聊了什么”、内容主线、今晚现实接话和避免追问点，不展示逐字聊天记录。
Rationale: 真机体验反馈显示父亲设置和日报过于机械，儿童端话题容易被追问过深，聊天页也需要更稳定的小白狐陪伴感。本轮优化目标是降低父亲设置负担、减少儿童被盘问感、让日报更像现实桥接工具。
...
Implementation notes: 年龄、年级、称呼偏好、兴趣和 topic boundaries 先写入 `communication_preferences`，避免新增 DB migration；topic seed pack 使用仓库静态 JSON，不抓实时网络热点；父亲日报新字段保持向后兼容，旧报告没有新字段时 Android 仍可展示；小白狐 UI 只增加 phase-derived 状态 chip 和轻背景，不引入积分、签到、排行榜或宠物机制。
Tests or QA needed: 后端单测覆盖 profile prompt、topic shift seeds、runtime 换题修复和父亲日报 topic overview；Android JVM 测试覆盖父亲设置 profile 保存、日报 DTO/UI 文案和 child UI chip label。Redmi K60 / Honor Pad 5 仍需验证设置页可读性、话题切换自然度、日报信息层级和横屏 UI polish。
...
Decision: Task 06 closeout 后，家庭内测 v0.1 继续收紧四点：隐藏 schedule/time-period 设置只作为后端兼容字段保留，父亲设置保存不得因为隐藏默认时间而校验失败或重写默认作息；topic seed pack 只能使用仓库内 reviewed、age-aware、带 safety_notes 和 expires_at 的静态 curated seeds，不做实时热点抓取；TTS 延迟必须通过后端 request_id timing 和 Android logcat timing 分解 model/TTS/audio playback；父亲日报继续只展示主题/内容摘要、现实接话和今晚避免追问，不提供 raw transcript 导出。
...
Implementation notes: schedule 仍随父亲策略 API 透传兼容旧数据，但 v0.1 UI 不展示且保存只保留后端已有 schedule；seed pack 每条 seed 必须包含 `id`、`label`、`age_bands`、`prompt_hint`、`safety_notes`、`expires_at`、`source=curated_v0_1`，过期或包含购买/排行/抽卡等 unsafe marker 的 seed 不返回；Android 只加轻量 idle fallback chips，不新增任务菜单、奖励或连续使用钩子；TTS timing logs 不记录儿童原文、完整回复、音频 URL query、parent_message_raw 或 provider secret。
Tests or QA needed: 自动测试覆盖 hidden schedule 不阻断保存、seed pack shape/expiry/safety、CS-like low engagement topic shift、stream/non-stream latency logs privacy、Android segment trace id propagation、CS/game father report summary。Redmi K60 / Honor Pad 5 仍需验证设置页、换题 chips、TTS timing 抓取和日报阅读层级。
...
15. Task 06/07 后，父亲设置以孩子画像和沟通偏好为主；显性作息配置从 v0.1 家庭内测 UI 中降级，隐藏 schedule 只做后端兼容且不重写默认时间；普通对话同题低能量时使用静态 reviewed seed 给换题选择；父亲日报以话题/内容摘要和现实接话为主，不做监控式原文回放。
```
