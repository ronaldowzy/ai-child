...
| PD-002 | revised | 语音输入 ASR v1 目标修订为后端接 MiMo audio input / ASR；Android 不直接调用 MiMo，只负责录音、上传后端和展示待确认文本；TTS 正式音色路径已由 PD-027 修订为后端 MiMo VoiceClone。 | Android 权限、语音设计、后端 API 边界、ASR、TTS |
...
| PD-010 | revised | 语音输入 v1 是 confirm-before-send：点击语音 -> 孩子说话 -> Android 录音上传后端 ASR -> 展示识别文本 -> 孩子确认/可编辑 -> 点击发送 -> text 走 conversation API。future hands-free conversational mode 不进入 v1。 | Android 语音输入、后端 ASR、QA、后端 API 边界 |
...
Decision: 语音输入 ASR v1 目标修订为后端接 MiMo audio input / ASR；Android 不直接调用 MiMo，只负责点击录音、上传到后端、展示待确认文本和确认后发送；TTS 正式音色路径已由 PD-027 修订为后端 MiMo VoiceClone。
...
Implementation notes: Android 仍必须 confirm-before-send，不做常开麦克风、不做识别后自动发送；后端 ASR provider 默认 disabled，真实儿童音频外发必须满足父亲授权和 `CHILD_AI_MIMO_ASR_*` policy flags；开发阶段只用 fake/smoke audio。
...
Date: 2026-05-19
Status: revised
Source: father / voice interaction decision; revised by MiMo ASR v1 confirmation on 2026-05-21
Decision: 语音输入 v1 是 confirm-before-send：点击语音 -> 孩子说话 -> Android 录音上传后端 ASR -> 展示识别文本 -> 孩子确认/可编辑 -> 点击发送 -> text 走 conversation API。future hands-free conversational mode 不进入 v1。
Rationale: 第一版优先可控和可纠错；连续自动对话需要更完整的误识别、安全、打断和收尾设计。ASR provider 改为后端 MiMo 后，确认步骤仍然不可省略。
Affected modules: Android voice、InputBar、ChatViewModel、ASR API、conversation API usage、QA。
Implementation notes: v1 不做自动发送、常开麦克风或唤醒词；Android 不保存 MiMo API key；ASR 返回的是 pending transcript，不是正式 child message。
Docs updated: `docs/VOICE_INTERACTION_DESIGN_V0_1.md`、`docs/NEXT_PHASE_PLAN_V0_2.md`、`android/README.md`。
...
Decision: 小白狐动态形象第一版采用父亲提供的 3D 风格 PNG 序列帧资源包，Android 运行时使用 manifest-driven `animation_v1` 播放；旧静态 PNG 和 Compose Canvas 必须继续作为 fallback。
...
Implementation notes: Android 运行时资源放在 `android/app/src/main/assets/mascot/xiaobaohu/v1/`，当前 manifest 声明 11 个状态、每状态 24 帧、12 FPS，assets 约 117MB；渲染 fallback 链为 `animation_v1 -> png_static -> canvas`；不接 Rive、不接实时 3D、不把 preview html/gif/webp/spritesheet 作为运行时依赖。
...
Decision: MiMo ASR / audio input 调研已完成，ASR v1 方案确定接 MiMo；实现顺序是后端 provider 和 policy gate 先行，Android 后续只做录音上传和待确认文本 UI。
...
Implementation notes: 继续保留 confirm-before-send 原则；真实儿童音频外发必须通过父亲授权和 ASR policy flags；Android 不保存模型 API key；开发 smoke 只允许 fake/smoke audio。
...
Decision: ASR v1 方案确定接 MiMo audio input / ASR。Android 不直接调用 MiMo，只负责点击录音、上传到后端、展示待确认文本；识别结果必须 confirm-before-send；不做常开麦克风，不做识别后自动发送。
...
2. 再进入语音输入 v1：后端接 MiMo ASR，Android 只负责录音上传和待确认文本 UI，先给孩子和父亲可见的文字确认，不自动发送误识别内容；hands-free conversational mode 留到未来阶段。
...
7. Android 第一版优先接预渲染 3D PNG/WebP 状态图和 animation_v1 PNG 序列帧，不引入实时 3D 引擎作为必需能力。
...
13. 下一阶段体验重点转向流式交互、横屏双栏、小白狐状态覆盖验证、MiMo ASR 后端接入与 Android confirm UI、运行基础组件补齐。
