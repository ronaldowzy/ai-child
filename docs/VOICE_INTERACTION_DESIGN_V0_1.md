...
4. Android TTS v1 已实现本地 TextToSpeech 抽象、默认自动朗读、停止/静音控制、VoiceProfile 和小白狐 speaking 状态联动；仍需在 Device A 与 Honor Pad 5 上做真实听感和性能 QA。
...
3. 朗读开始时临时把小白狐切到 speaking 状态，并映射到 fox_3d_speaking。
4. 朗读结束或停止后恢复后端 reply 对应的 base mood / motion。
5. UI 提供停止和静音控制；DevSettings.AUTO_TTS_ENABLED 默认 true。
6. 系统 TTS 不可用时显示“我现在不能朗读，但文字还在这里。”并降级为文字。
7. 当前不生成、不保存任何音频文件，不接外部 TTS 服务。
8. 真实设备听感、中文 voice 可用性、延迟和 Honor Pad 5 低配表现仍需 QA。
```
