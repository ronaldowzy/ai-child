...
用途：定义儿童端语音输入、TTS 朗读、音频数据边界和 QA 验收清单。本文档只描述已确认方向和下一阶段计划，不表示当前 Android 已实现真实语音。
...
3. Android 语音入口仍是占位或后续入口，不录音、不调用真实 SpeechRecognizer、不播放真实 TTS。
4. mock 拍题和学习求助仍以文字和 mock OCR 为主。
```
...
5. TTS 播放时，小白狐可以进入 speaking 状态；停止或静音后恢复 idle / neutral。
