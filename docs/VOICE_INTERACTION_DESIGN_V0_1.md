...
用途：定义儿童端语音输入、TTS 朗读、音频数据边界和 QA 验收清单。本文档区分已实现能力和待设备验收项；Android 语音输入 v1 已实现录音上传、后端 ASR 和待确认文本 UI，仍待 Redmi K60 / Honor Pad 5 真机 QA。小白狐语音输出主路径已从 Android 系统 TTS 调整为后端 MiMo VoiceClone 音频生成，Android 系统 TTS 保留为 fallback 和诊断能力。

关联决策：

```text
docs/PRODUCT_DECISIONS_V0_1.md
PD-002：语音输入 ASR v1 目标修订为后端接 MiMo audio input / ASR；Android 不直接调用 MiMo，只负责录音、上传后端和展示待确认文本；TTS 正式音色路径已由 PD-027 修订。
...
PD-010：语音输入 v1 是 confirm-before-send，Android 录音上传后端 ASR 后必须展示可编辑文本，future hands-free conversational mode 不进入 v1。
...
3. Android 语音输入 v1 已接入：点击后请求麦克风权限，录制短 WAV，上传后端 `/api/v1/asr/transcribe`，展示可编辑待确认文本。
...
```text
1. 语音输入 v1：Android 点击录音后上传短音频到后端 ASR，后端在 policy gate 允许时调用 MiMo audio input / ASR。
2. 识别结果必须先展示为可确认文本，由孩子或父亲确认后再发送 conversation API。
...
5. 后端 ASR endpoint 可以接收一次性短音频做转写，但原始音频不入长期库、不进日志、不进入 memory；语音输入正式进入对话的仍是确认后的文本。
6. Hands-free conversational mode 是 future，不进入 v1。
7. MiMo ASR / audio input 已完成第一轮规格调研，结论见 `docs/ASR_INPUT_RESEARCH_V0_1.md` 和 `docs/MIMO_ASR_INTEGRATION_DESIGN_V0_1.md`；ASR v1 目标确定接 MiMo，但真实儿童音频外发必须由父亲授权和 ASR policy flags 控制。
...
| SpeechInputController | 处理点击语音、录音、上传后端 ASR、识别结果、取消、错误和 confirm-before-send 状态 | Android recorder + backend ASR upload wrapper |
| TtsController | 处理自动朗读、停止、静音、远程音频播放、系统 TTS fallback、错误和生命周期释放 | Remote audio player + Android TextToSpeech fallback |
...
```text
1. 第一阶段 ASR provider 使用后端 MiMo audio input / ASR；Android 平台 SpeechRecognizer 可保留为未来 fallback 评估，不是当前 ASR v1 主路径。
2. 语音识别只作为文字输入辅助，不改变后端统一 conversation API 的安全边界。
...
3. MiMo ASR 只能返回待确认文本；孩子确认或编辑后，才走 conversation API。
...
点击语音
  ...
  -> 后端按 policy gate 调用 MiMo ASR 或返回可控失败
  -> 展示识别文本
  -> 孩子确认 / 可编辑 / 可重说 / 可取消
  -> 点击发送
...
## 4. Backend And Android TTS Strategy
...
7. 如果 reply.audio_url 为空或播放失败，Android 使用本地系统 TTS fallback；系统 TTS 只作诊断和降级，不作为正式品牌音色。
...
```text
1. 语音输入当时不可用：旧 APK 反馈已归档；当前 Android ASR v1 已接录音上传、后端 `/api/v1/asr/transcribe` 和待确认文本 UI，仍需新 APK 真机复验。
...
`VoiceProfile` 用于集中管理 Android 系统 TTS fallback 的调优。它不再代表正式小白狐品牌音色；正式主路径由后端 MiMo VoiceClone 生成音频。
...
5. 系统 TTS 不承诺固定专属音色；如果系统 TTS 效果不好，记录 QA 并 fallback 为文字或后端 audioUrl。
```
...
当前已经新增后端 TTS 输出接口，但仍不新增孩子原始音频上传接口。
...
2. `/api/v1/asr/transcribe` 只能返回 `requiresConfirmation=true` 的 pending transcript。
...
| 无网络或后端 ASR 不可用 | 现在语音可能用不了，可以先打字，或者请大人帮忙看看网络。 |
...
| 设备 | 定位 | 用途 |
|---|---|---|
| Device A：Redmi K60，Android 14 | 功能主验证 | 快速验证后端 ASR 录音上传、远程 audioUrl 播放、系统 TTS fallback、小白狐状态切换、图片资源和轻量动画、真实模型/Mock 模型对话体验，以及自由聊天、学习求助、直接要答案、安全场景、隐私边界和父亲入口保护等核心流程 |
| Device B：Honor Pad 5，Android 9，RAM 4GB | 低配兼容性目标设备 | 验证 Android 9 兼容性、4GB 内存性能、平板横屏/大屏 UI、儿童真实使用尺寸、后端 ASR 上传确认、TTS 可用性、小白狐资源大小、动画流畅度、发热和卡顿 |

执行顺序：

```text
1. V1 语音输入先在 Device A 跑通点击语音 -> 录音 -> 上传后端 ASR -> 展示文字 -> 确认/编辑 -> 发送。
2. 再在 Honor Pad 5 验证权限申请、录音格式、中文识别、儿童声音识别、延迟、失败提示和是否可接受。
3. V2 TTS 先在 Device A 跑通 `reply.audio_url` 远程音频播放、默认自动朗读、停止、关闭和系统 TTS fallback。
4. 再在 Honor Pad 5 验证远程 wav 播放、缓存音频体积、播放延迟、是否卡顿、系统 TTS fallback 是否可用，以及是否需要关闭自动朗读作为低配默认。
...
3. 中文普通话识别效果。
4. 儿童声音识别效果。
5. 后端 VoiceClone 或系统 TTS fallback 的自然度。
6. 孩子接受度。
```
