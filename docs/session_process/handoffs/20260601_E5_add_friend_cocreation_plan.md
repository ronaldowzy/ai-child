...
以下 3 句话术已由主控确认为 E5 master-copy 精确文本：

| 话术 | 用途 | 状态 |
|---|---|---|
| 那我们给它找一个小伙伴\n你可以说一个名字，也可以给我看看 | 点击"加一个朋友"后的引导话术 | **已确认** |
| {name}，也来小屋里待一会儿啦 | 起名成功后的反馈话术 | **已确认** |
| 说个名字 / 给小白狐看看 / 先聊别的 | 引导话术后的按钮文案 | **已确认** |

safe_summary 追加格式已确认：`"孩子给小屋小客人加了一个小伙伴：{name}"`

quick_action_id 已确认：`companion_friend_name` / `companion_friend_image` / `companion_skip`（复用现有跳过逻辑）
