...
if [[ "${1:-}" != "--yes" ]]; then
  echo "This resets the local dev database schema. Re-run with --yes to continue." >&2
  exit 2
fi
...
resolve_python_cmd
cd "${BACKEND_DIR}"
echo "Resetting local database schema with Alembic"
"${PYTHON_CMD[@]}" -m alembic -c alembic.ini downgrade base
"${PYTHON_CMD[@]}" -m alembic -c alembic.ini upgrade head
